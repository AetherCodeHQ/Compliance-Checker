...
> Compliance-Checker - Open source tool by AetherCodeHQ

`cli` `compliance` `devops` `golang` `security`

## What is it?

**Compliance Checker** is a security tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Compliance-Checker.git
...
```bash
# Basic usage
./Compliance-Checker --path ./target

# With options
./Compliance-Checker --path ./target --format json --output report.json

# Verbose mode
./Compliance-Checker --path ./target --verbose
```
...
### Basic scan
```bash
./Compliance-Checker --path ./my-project
```

### JSON report
```bash
./Compliance-Checker --path ./my-project --format json --output report.json
```
...
- name: Run Compliance Checker
  run: ./Compliance-Checker --path . --format json --output report.json
```
